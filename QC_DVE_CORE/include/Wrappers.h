...
namespace wrappers {
    ...
    PyObject *show(qf::CompoundedOvernightRateCashflow cshflw) {
        // The types inside the wrapper are:
        // startDate 0,
        // endDate 1,
        // settlementDate 2,
        ...
        try {
            ...
            cashflowAmount = std::get<7>(*cashflow);
            currency = std::get<8>(*cashflow)->getIsoCode();
            indexCode = std::get<9>(*cashflow);
            rateValue = std::get<13>(*cashflow);
        ...
        success = PyTuple_SetItem(result, 7, PyFloat_FromDouble(cashflowAmount));
        if (success != 0) {
            PyObject *qcfError = PyErr_NewException("QC_Financial Error", NULL, NULL);
            PyErr_SetString(qcfError, "total flow");
            return NULL;
        ...
        const char *cIndexCode = indexCode.c_str();
        success = PyTuple_SetItem(result, 9, PYSTRING2STRING(cIndexCode));
        if (success != 0) {
            PyObject *qcfError = PyErr_NewException("QC_Financial Error", NULL, NULL);
            PyErr_SetString(qcfError, "ir indexCode");
            return NULL;
        }
        ...
        success = PyTuple_SetItem(result, 13, PYSTRING2STRING((wf + yf).c_str()));
        if (success != 0) {
            PyObject *qcfError = PyErr_NewException("QC_Financial Error", NULL, NULL);
            PyErr_SetString(qcfError, "type of rate");
            return NULL;
    ...
    PyObject *show(std::shared_ptr<qf::CompoundedOvernightRateCashflow> cshflwPtr) {
        // The types inside the wrapper are:
        // startDate       QCDate,
        // endDate         QCDate,
        // fixingDate      QCDate,
    ...
    boost::python::tuple getColumnNames(const std::string &cashflowType, const std::string &cashflowSubtype = "") {
        if (cashflowType == "FixedRateCashflow" || cashflowType == "FixedRateCashflow2") {
            return boost::python::make_tuple("fecha_inicial", "fecha_final", "fecha_pago", "nominal",
                                             "amortizacion", "interes", "amort_es_flujo", "flujo", "moneda",
            ...
            return boost::python::make_tuple("fecha_inicial", "fecha_final", "fecha_pago", "nominal",
                                             "amortizacion", "amort_es_flujo", "flujo", "moneda", "icp_inicial",
                                             "icp_final",
