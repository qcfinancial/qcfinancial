...
State invariants for `CompoundedOvernightRateCashflow2`'s cached equivalent-rate wealth
factor (`_endDateWf`). Covers what `fixing()` stores for a realized coupon, why `amount()`
stays unrounded and differentiable, and the requirement that every valuation and reporting
method report interest from the same rounded equivalent rate for a fully realized coupon.
Established in change `fix-compounded-overnight-eq-rate-state` (v1.12.0a4).
