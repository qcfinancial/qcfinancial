...
- Add a new **`BusinessCalendarId`** enum whose symbols are FpML business-center codes: `CLSA` (Santiago, Chile — public holidays), `CLBA` (Chile banking calendar = CLSA + Dec 31), `USNY` (US New York banking / SIFMA settlement), `USGS` (US government securities / SIFMA bond market), `EUTA` (Eurozone TARGET). (`CLBA` is a house code; FpML has no canonical Chile-banking code.)
...
- **Out of scope (v1):** non-Sat/Sun weekends (Fri/Sat Middle-East calendars) and any calendar beyond the five listed above.
