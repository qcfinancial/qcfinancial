...
QCDate QCode::Financial::IborCashflow2::endDate() {
    return _endDate;
}
...
double
QCode::Financial::IborCashflow2::accruedFixing(const QCDate &fecha, const QCode::Financial::TimeSeries &fixings) {
    if (fecha < _startDate || _endDate >= fecha) {
        return 0.0;
    }
    try {
...
std::shared_ptr<QCode::Financial::IborCashflow2::IborCashflow2Wrapper> QCode::Financial::IborCashflow2::wrap() {
    QCode::Financial::IborCashflow2::IborCashflow2Wrapper tup = std::make_tuple(
            _startDate,
            _endDate,
            _fixingDate,
            _settlementDate,
