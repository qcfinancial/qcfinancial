	...
	try
	{
		...
		for (unsigned long i = 0; i < PyList_Size(legCharacteristics); ++i)
		{
            ...
            auto stringDate2 = string(PyString_AsString(PyList_GetItem(
                    PyList_GetItem(legCharacteristics, i), 3)));
            QCInterestRateLeg tmpIntRateLeg = QCFactoryFunctions::buildFrenchFixedRateLeg(
				...
				QCDate{stringDate1}, //start date
				QCDate{stringDate2}, //end date
...
//funcion __init__ python, acá estamos construyendo el modulo pyd
PyMODINIT_FUNC
